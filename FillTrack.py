...
# @Software: PyCharm
"""
1、实现待补路段的查找，每个车辆的待补路段均保存为一个txt文件，文件名为车牌号
2、待补路段完成后，查找每个路段的的相似区域，保存格式txt
3、补充缺失路段轨迹，保存文件为csv格式，每个车辆有两个文件，一个为补充点的文件，一个是原始坐标点+补充点，字段为经纬度，坐标标识
此部分补充策略为迭代轨迹补充，即先选出坐标点最多的补充车辆，如果其坐标点数少于10个，继续选择第二个，依次类推，直到补充点坐标数达到10个及以上
"""
import pandas as pd
import math
import os
...
df = pd.read_csv(os.path.join(Trunkpath,Filledcsvname),header=None,usecols=[2,3],names=[0, 1])  
